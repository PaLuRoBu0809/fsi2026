 ...
 */
public class TareaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tareas").get("/api/tareas").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tarea")
                        .post("/api/tareas")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"titulo\": \"SAMPLE_TEXT\"" + ", \"descripcion\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tarea").get("${new_tarea_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created tarea").delete("${new_tarea_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Tarea entity").exec(scn);
