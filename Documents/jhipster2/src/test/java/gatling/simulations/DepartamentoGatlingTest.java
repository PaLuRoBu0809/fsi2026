 ...
 */
public class DepartamentoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all departamentos").get("/api/departamentos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new departamento")
                        .post("/api/departamentos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"nombreDepartamento\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_departamento_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created departamento").get("${new_departamento_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created departamento").delete("${new_departamento_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Departamento entity").exec(scn);
