...
public class PaisGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all pais").get("/api/pais").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pais")
                        .post("/api/pais")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"nombrePais\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created pais").get("${new_pais_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created pais").delete("${new_pais_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
