 ...
 */
public class TrabajoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trabajos").get("/api/trabajos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trabajo")
                        .post("/api/trabajos")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"tituloTrabajo\": \"SAMPLE_TEXT\"" + ", \"salarioMin\": 0" + ", \"salarioMax\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_trabajo_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trabajo").get("${new_trabajo_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created trabajo").delete("${new_trabajo_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Trabajo entity").exec(scn);
