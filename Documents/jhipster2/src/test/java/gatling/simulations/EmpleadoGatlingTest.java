...
public class EmpleadoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all empleados").get("/api/empleados").headers(headersHttpAuthenticated).check(status().is(200)))
                    ...
                    http("Create new empleado")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"correo\": \"SAMPLE_TEXT\"" +
                                ", \"nroCelular\": \"SAMPLE_TEXT\"" +
                                ", \"fechacontrato\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"salario\": 0" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created empleado").get("${new_empleado_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created empleado").delete("${new_empleado_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
